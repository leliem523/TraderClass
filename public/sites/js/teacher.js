  ...
  $('.btn_subcribe_teacher').click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if (!data.status) {
          data.error.course_category_id != undefined
            ? $('.error_cate').html(data.error.course_category_id).show()
            : '';
          data.error.email != undefined
            ? $('.error_input').html(data.error.email).show()
            : '';
          data.error.agree_chk != undefined
            ? $('.error_agree').html(data.error.agree_chk).show()
            : '';
        } else {
          swal(
            'Sucessfuly, Thank you!',
            "We're will contact soon!",
            'success'
          ).then((value) => {
            if (value) {
              location.reload();
